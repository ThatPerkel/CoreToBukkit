...
public class BukkitListener implements Listener {
    ...
    public void onSignChangeEvent(SignChangeEvent event){
        ...
        call(event1);
        for(int A = 0; A < 4; A++) {
            event.setLine(A, event1.getTo().getLine(A));
        }
    ...
    private Set<BEventLaunch> getMethods(Class<? extends Event> classEvent){
        ...
        CorePlugin.getEventManager().getEventListeners().entrySet().stream().forEach(e -> e.getValue().stream().forEach(el -> {
            for (Method method : el.getClass().getDeclaredMethods()){
                ...
                if(!Event.class.isAssignableFrom(classEvent)){
                    System.err.println("Failed to know what to do: HEvent found on method, but no known event on " + el.getClass().getName() + "." + method.getName() + "(" + CorePlugin.toString(", ", p -> p.getType().getSimpleName() + " " + p.getName(), method.getParameters()) + ")");
                }
